...
def agendar_clase(request):
    if request.method == 'POST':
        ...
        if cancelar:
            ...
            ahora = timezone.now()
            diferencia = clase.horario - ahora
            
            if diferencia <= timedelta(hours=24):
                messages.error(request, "Solo puedes cancelar una clase con al menos 24 horas de anticipación.")
...
def lista_entrenadores_cotizacion(request):
